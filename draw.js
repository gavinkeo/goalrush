...
const CLUB_SEARCH_ALIASES={
  ...
  "athletic bilbao":["Athletic Bilbao","Athletic Club"],"rangers":["Rangers","Glasgow Rangers"],"ferencvaros":["Ferencvaros","Ferencváros"],
  "midtjylland":["Midtjylland","FC Midtjylland"],"rapid wien":["Rapid Wien","Rapid Vienna"],"viktoria plzen":["Viktoria Plzen","Viktoria Plzeň"],
  "legia warsaw":["Legia Warsaw","Legia Warszawa"],"copenhagen":["Copenhagen","FC Copenhagen","FC København"],"maccabi tel aviv":["Maccabi Tel Aviv","Maccabi TA"],
...
async function seedLocalCrestsFromCompetition(){
  try{
    const r=await fetch("competition.json?v=117",{cache:"no-store"});
    if(!r.ok)return;
...
function setCrest(img,team){img.src=cachedCrest(team);if(team&&!crestCache[clubKey(team)])hydrateCrest(team,img)}
async function hydrateCrest(team,img){const key=clubKey(team);if(!key)return;if(crestCache[key]){img.src=crestCache[key];return}if(crestLookupsInFlight.has(key)){const badge=await crestLookupsInFlight.get(key);if(badge)img.src=badge;return}const lookup=(async()=>{try{const badge=await fetchCrestForClub(team);if(badge){crestCache[key]=badge;saveCrestCache();return badge}}catch(error){console.warn(`Draw crest lookup failed for ${team}:`,error)}return null})();crestLookupsInFlight.set(key,lookup);const badge=await lookup;crestLookupsInFlight.delete(key);if(badge){img.src=badge;renderResults()}}
