...
function mobileCard(entry, rank, rankLabel = String(rank), prizeLabel = "") {
  ...
  const managerName = desktopManagerNameParts(entry.entrant);
  const firstLine = managerName.first || managerName.last;
  const secondLine = managerName.first ? managerName.last : "";

  const clubCell = (team, comp) => `
    <span class="v148-club-score ${comp.toLowerCase()}">
      <img class="v148-club-crest"
           data-club-key="${esc(clubKey(team.club))}"
           src="${crest(team)}"
           alt="${esc(team.club)}"
           onerror="this.src='${PLACEHOLDER_CREST}'">
  ...
  return `
    <details class="v148-entry" data-entrant="${esc(entry.entrant)}">
      <summary class="v148-row" aria-label="${esc(entry.entrant)} — ${totalScore(entry)} goals">
        <span class="v148-rank">${rankBlock(rank, rankLabel, prizeLabel)}</span>
        <span class="v148-name">
          <span>${esc(firstLine)}</span>
          ${secondLine ? `<span>${esc(secondLine)}</span>` : ""}
        </span>
        ${clubCell(entry.ucl, "UCL")}
        ${clubCell(entry.uel, "UEL")}
        <span class="v148-total">${totalScore(entry)}</span>
      </summary>
      <div class="v148-details">
        ${row(entry.ucl, "UCL")}
        ${row(entry.uel, "UEL")}
      </div>
    </details>`;
...
function render() {
  ...
  mobileEl.innerHTML = `
    <div class="v148-mobile-leaderboard">
      <div class="v148-header" aria-hidden="true">
        <span>Rank</span>
        <span>Name</span>
